 ...
 * @return {object} filename, header, and body content
 */
function parseSection(text, urlDestination, textDestination) {
    ...
    body = body.replace(/^#{2,}\s*(.*)/m, function(match, id) {
        header = _.kebabCase(id.toLowerCase());
        return '';
    });
    ...
    return {
        filename: textDestination(header),
        header: header,
        body: _.trim(body) + '\n',
    };
}
...
Textventure.prototype.parseSection = function(text) {
    return parseSection(text, _.bind(this.idToDestinationUrl, this), _.bind(this.idToDestinationFilename, this));
};
